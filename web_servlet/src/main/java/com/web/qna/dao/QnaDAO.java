...
//			pstmt = con.prepareStatement(LIST);
	...
	public int write(QnaVO vo) throws Exception {
		...
		try {
			...
			e.printStackTrace();
			throw new Exception(" 예외 발생: 질문답변 등록 DB처리 중 예외가 발생했습니다. ");
	...
	final String UPDATE = "update Qna set " + " title = ?, content = ? ,writer = ? " + " where no = ? and pw = ? ";
