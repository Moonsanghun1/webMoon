		...
		<form action="newList.do" id="searchForm">
			...
			<thead>
				...
				<c:forEach items="${newList}" var="vo">
					<c:choose>
						<c:when test="${!empty login && login.gradeNo == 9 || vo.musicStatus == '공개'}">
							<tr class="dataRow ${vo.musicStatus == '비공개' && (empty login || login.gradeNo != 9) ? 'disabled' : ''}" data-music-no="${vo.musicNo}">
								...
								<td>
									<!-- 담기 버튼 --> <span class="btn-add" data-music-no="${vo.musicNo}" data-album-no="${vo.albumNo}">&nbsp;&nbsp;<i class="fa fa-plus"></i></span>
								</td>
							</tr>
						</c:when>
						<c:otherwise>
				...
				<c:if test="${!empty login && login.gradeNo == 9 }">
					<tr>
						<td colspan="5">
							<!-- 등록 버튼 --> <a href="writeForm.do" class="btn btn-dark">등록</a>
						</td>
					</tr>
				</c:if>
			</table>
