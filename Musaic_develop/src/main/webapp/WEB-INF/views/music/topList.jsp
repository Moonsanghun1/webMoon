		...
		<table class="table">
			...
			<tbody>
				<c:forEach var="music" items="${musicList}" varStatus="status">
					<tr class="dataRow ${music.musicStatus == '비공개' ? 'disabled' : ''}"
						data-music-no="${music.musicNo}"
						data-toggle="${music.musicStatus == '비공개' ? 'tooltip' : ''}"
						data-placement="top"
						title="${music.musicStatus == '비공개' ? '이 곡은 비공개 상태입니다.' : ''}">
						<td
							style="font-size: larger; font-family: sans-serif; color: #7F8BA6">
							<strong>&nbsp;&nbsp;${status.index + 1}</strong>
						</td>
						<td><img src="${music.image}" alt="${music.musicTitle}"
							style="width: 40px; height: 40px; margin-right: 10px;" />
							${music.musicTitle}</td>
						<td>${music.singer}</td>
						<td>
							<!-- 담기 버튼 --> <span
							class="btn-add ${music.musicStatus == '비공개' ? 'disabled' : ''}"
							data-music-no="${music.musicNo}">&nbsp;&nbsp;<i
